...
| Pydantic | `>=2.11.10,<2.12.6` | Matches the current HF Spaces Gradio builder, which installs `gradio[mcp,oauth]==6.17.3` and caps Pydantic at `<=2.12.5`. |
| PyTorch / torchvision | `torch>=2.8,<2.12`, `torchvision>=0.25,<0.27` | Matches the current HF Spaces builder cap of `torch<=2.11.0`; `torchvision 0.27.0` requires `torch==2.12.0`. |
